...
describe('EventsTable', () => {
  ...
  it('renders dropdown menu for actions', () => {
    ...
    const dropdownTriggers = actionButtons.filter(button => 
      button.querySelector('svg')?.getAttribute('class')?.includes('MoreHorizontal') ||
  ...
  it('calls onUpdateStatus when status is changed', () => {
    ...
    const actionButtons = screen.getAllByRole('button')
    const firstDropdownTrigger = actionButtons.find(button => 
      button.getAttribute('aria-haspopup') === 'menu'
    )
  ...
  it('displays empty state when no events', () => {
    ...
    expect(screen.getByText('No events found matching the selected filter.')).toBeInTheDocument()
  })
